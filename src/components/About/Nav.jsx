                ...
                <>
                                    ...
                                    <div className="flex space-x-4 pt-4">
                                        {navigation.map((item) => (
                                            <NavLink
                                                to={item.to}
                                                key={item.name}
                                                className={({ isActive, isPending }) => classNames('px-3 py-2 rounded-md font-medium cursor-pointer',
                                                    isActive ? 'text-white bg-accelloBlue' : isPending ? 'text-[#000000]' : 'hover:bg-accelloBlue hover:text-white'
                                                )}
                                            >
                                                {item.name}
                                            </NavLink>
                    ...
                    <Disclosure.Panel className="sm:hidden">
                        <div className="block space-y-6 px-2 pt-2 pb-3 mt-8">
                            {navigation.map((item) => (
                                <NavLink to={item.to}
                                            className={({ isActive, isPending }) => classNames('w-full px-3 py-2 rounded-md text-sm font-medium cursor-pointer',
                                            isActive ? 'text-white bg-accelloBlue w-full' : isPending ? 'text-[#000000] hover:bg-accelloBlue hover:text-white' : ''
                                        )}>
                                    <Disclosure.Button
                                        key={item.name}
                                        as="p" className='w-full'>
                                            {item.name}
                                    </Disclosure.Button>
                                </NavLink>
                            ))}
                            ...
                            <div className='flex flex-col w-[200px] gap-5'>
                                ...
                                <DomLink to="/sign-in">
                                    <button type="button" className="inline-flex border-2 border-accelloBlue rounded-[8px] bg-transparent py-4 px-8 text-accelloBlue hover:text-accelloBlue gap-2 text-center  " >
                                        <LockClosedIcon className="block h-5 w-5" aria-hidden="true"/>
                                        Login
                                    </button>
                                </DomLink>
