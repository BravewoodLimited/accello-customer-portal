...
function Calculator({ handleClickOpen }) {
  ...
  const totalRepayment =
    parseFloat(amountInput) + parseFloat(interestamount * sliderValue);
    ...
    <div className="p-4 bg-landingbackground" name="calculator">
      ...
      <div className="sm:flex gap-4 flex-row mx-auto max-w-7xl px-2 sm:px-6 lg:px-8 justify-between mt-8 ">
        <div className="sm:max-w-[50%] mt-12">
          <h1 className="font-NexaBold lg:text-[36px] md:text-[28px] text-[22px] font-bold">
            Personal Loan Calculator
          </h1>

          <p className="sm:w-[400px] text-[#4D4D4D]">
          ...
          <Tab.Group>
            <Tab.List className="hidden space-x-6 rounded-xl p-1">
              {Object.keys(categories).map((category, index) => (
                <Tab
                  key={index}
                  onClick={({ selected }) =>
                    selected ? setloanType(selected) : setloanType(category)
                  }
                  className={({ selected }) =>
                    classNames(
            ...
            <Tab.Panels className="mt-12">
              {Object.values(categories).map((posts, idx) => (
                <Tab.Panel key={idx} className={classNames("p-3 space-y-16")}>
                  <TextField
                    id="standard-basic"
                    key={idx}
                    label="Amount"
        ...
        <div className="sm:max-w-[50%] mt-12 bg-white sm:w-[548px] rounded-[25px] ">
          ...
          <div className="mx-auto max-w-7xl flex items-center justify-center">
            ...
            <button
              type="button"
              className="border-2 border-white rounded-[8px] bg-accelloBlue py-3 px-16 text-white gap-2 mt-6 text-[16px] w-auto text-center items-center mx-auto mb-10 hover:bg-transparent hover:border-accelloBlue hover:text-accelloBlue "
            >
              Apply for Loan 
            </button>
            </DomLink>
